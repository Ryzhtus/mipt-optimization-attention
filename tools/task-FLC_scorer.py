...
def check_data_file_lists(submission_annotations, gold_annotations, task_name="task3"):
    ...
    if len(gold_annotations) < len(submission_annotations):
        logger.error("The number of articles in the submission, %d, is greater than the number of articles in the "
                     "reference dataset" % (len(submission_annotations), len(gold_annotations))); sys.exit()

    #check that all file names are correct
    errors = [article_id for article_id in submission_annotations.get_article_id_list()
              if article_id not in gold_annotations.get_article_id_list()]
    if len(errors)>0:
...
def compute_score_pr(submission_annotations, gold_annotations, technique_names, prop_vs_non_propaganda=False,
    ...
    for article_id in submission_annotations.keys():
        gold_data = [ g in gold_annotations[article_id] if g[0] in technique_names ]
        logger.debug("Computing contribution to the score of article id %s\nand tuples %s\n%s\n"
                     % (article_id, str(submission_annotations[article_id]), str(gold_data)))

        article_cumulative_Spr_prec, article_cumulative_Spr_rec = (0, 0)
        for j, sd in enumerate([ s in submission_annotations[article_id] if s[0] in technique_names ]): #submission annotations for article article_id:
            s=""
            sd_annotation_length = len(sd[1])
            for i, gd in enumerate(gold_data):
                if prop_vs_non_propaganda or gd[0]==sd[0]:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scorer for FLC task on the Propaganda Techniques Corpus. " 
                                     + "If only -s option is provided, only some checks on the submitted file is performed.")
    ...
    parser.add_argument('-t', '--propaganda-techniques-list-file', dest='techniques_file', required=False, 
                        default=TECHNIQUE_NAMES_FILE, help="file with the list of propaganda techniques")
    ...
    parser.add_argument('-m', '--merge-user-annotations', dest='merge_user_annotations', required=False, action='store_true',
                        default=False, help="If the option is added, overlapping user annotations are merged into one")
